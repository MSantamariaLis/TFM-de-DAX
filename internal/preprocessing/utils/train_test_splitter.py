...
def temporal_combine(df_train: pd.DataFrame, df_test: pd.DataFrame, ids_col: str, date_col: str) -> pd.DataFrame:
    """
    Combines train and test DataFrames, restoring original order if possible.
    ...
    -------
    pd.DataFrame
        Combined DataFrame with original order restored.
    """
    
    if df_train is None and df_test is None:
        return None
    
    df_full = pd.concat([df_train, df_test]).sort_values(by=[ids_col, date_col])
